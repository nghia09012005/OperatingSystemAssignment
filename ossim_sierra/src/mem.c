...
static int translate(
	...
	if (proc == NULL || physical_addr == NULL) return 0;
...
int free_mem(addr_t address, struct pcb_t * proc) {
	...
	for (int i = 0; i < trans_table->size; i++) {
		if (trans_table->table[i].v_index == second_lv) {
			...
			for (int j = i; j < trans_table->size - 1; j++) {
...
int read_mem(addr_t address, struct pcb_t * proc, BYTE * data) {
	if (proc == NULL || data == NULL) return 1;
...
void dump(void) {
	...
	for (int i = 0; i < NUM_PAGES; i++) {
		if (_mem_stat[i].proc != 0) {
			printf("Page %d: Process %d, Index %d, Next %d\n",
				   i, _mem_stat[i].proc, _mem_stat[i].index, _mem_stat[i].next);
		}
	}
}

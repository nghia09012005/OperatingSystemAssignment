...
int inc_vma_limit(struct pcb_t *caller, int vmaid, int inc_sz)
{
  ...
  // // Xác định vùng cần cấp phát thêm
  ...
  // Cập nhật vm_end (tăng giới hạn vùng nhớ)
  cur_vma->vm_end = area->rg_end;

  // Cập nhật sbrk (vị trí tiếp theo sẽ cấp phát nếu gọi tiếp)
  cur_vma->sbrk = area->rg_end;

  // Ánh xạ vùng nhớ mới vào RAM (mỗi trang một frame)
